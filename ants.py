...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place_now.entrance != None:
            if random_bee(place_now.bees) != None:
                if self.lower_bound <= distance <= self.upper_bound:
                    return random_bee(place_now.bees)
                else:
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if random_bee(self.place.bees):
                the_random_bee = random_bee(self.place.bees)
                the_random_bee.reduce_health(the_random_bee.health)
        ...
        while current_place.exit != None:
            ...
            if current_place.ant != None:
                if current_place.ant.is_container:
                    if current_place.ant.ant_contained != None and current_place.ant.damage == 0:
                        current_place.ant.ant_contained.double()
                    elif current_place.ant.ant_contained != None:
                        current_place.ant.ant_contained.double()
